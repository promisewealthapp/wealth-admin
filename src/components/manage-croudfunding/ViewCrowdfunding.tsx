...
const ViewCrowdfunding = ({ record, type }: TViewCrowdfundingProps) => {
    ...
    }, [crowdLoading, crowdSuccess, currentLoading, currentSuccess])
    ...
    return (
        <div className='min-w-[320px] md:w-[660px] pt-2'>
            <img src={record?.thumbnail} alt={record?.title} className="w-full h-36 md:h-[280px] 2xl:h-[300px] rounded-lg" />
            <div className='flex flex-col md:flex-row md:items-center justify-between pt-2'>
                <div className=''>
                    ...
                    <h2 className="text-[#181818] font-semibold text-xl">{record?.title}</h2>
                </div>
                <div className='pt-1'>
            ...
            <div className='grid grid-cols-2 md:flex items-center gap-2 md:gap-10'>
                ...
                <div className='col-span-2'>
                    ...
                    <h2 className="text-textDark font-medium">{record?.streetLocation}</h2>
                </div>
            </div>
                    ...
                    <div className='flex items-center justify-between py-2'>
                        <div className='flex flex-col'>
                            ...
                            <div className='pt-1 flex items-center gap-1'>
                                ...
                                <h2 className="text-textDark  font-medium">{record?.fundRaised}</h2>
                            </div>
                        </div>
                        <div className='flex flex-col'>
                            ...
                            <div className='pt-1 flex items-center gap-1'>
                                <FaNairaSign className="text-textDark" />
                                <h2 className="text-textDark font-medium">{record?.targetFund}</h2>
                            </div>
                        </div>
                    </div>

                    <Progress percent={(record?.fundRaised && record?.targetFund) ? Number(((record?.fundRaised / record?.targetFund) * 100)?.toFixed(1)) : 0} strokeColor={"#EC9414"} />
                </>
            }
            ...
            {(record?.order && record?.order[0]) && (
                <div className=''>
                    ...
                    <div className='py-1 flex gap-1'>
                        <div className=''>
                            <img className="size-8 rounded-full" src={record?.order[0]?.orderBy?.profileImg} alt={record?.order[0]?.orderBy?.name} />
                        </div>
                        <div className=''>
                            <h2 className="text-textDark font-medium capitalize">{record?.order[0]?.orderBy?.name}</h2>
                            <p className="text-xs text-[#6B6B6F]">{record?.order[0]?.orderBy?.role}</p>
                            <p className="text-sm">Email: {record?.order[0]?.orderBy?.email}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className='pt-2  space-y-2 md:space-y-4'>
                <div className='grid md:grid-cols-2 gap-2 md:gap-4'>
                    <div className='w-full'>
                        ...
                        <img src={record?.thumbnail} alt="" className="rounded-lg w-full h-40" />
                    ...
                    <div className='relative w-full'>
                        ...
                        {record?.videoUrl && (
                            <iframe
                                ...
                                title="YouTube Video"
                                src={`https://www.youtube.com/embed/${record?.videoUrl?.split('/')?.pop()?.split('?')[0]}`}
                                allowFullScreen
                            ></iframe>
                        )}
