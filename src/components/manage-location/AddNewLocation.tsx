...
const AddNewLocation = ({ closeModal }: TAddNewLocation) => {
    ...
    return (
        <div className='min-w-[320px] md:w-[560px]'>
            <form className="space-y-4 2xl:space-y-6 py-2" onSubmit={handleSubmit(onSubmit)}>
                <input onChange={(e) => handleFileUpload(e.target.files && e.target.files[0])} type="file" name="" id="imgUrl" className="hidden" />
                <div className='flex flex-col items-center gap-1 justify-center'>
                    {
                        loading ? <div className="rounded-full border border-[#E3E7E7] w-[100px] h-[100px]"> <SmallLoading /></div>
                            : <label htmlFor="imgUrl" className="rounded-full cursor-pointer w-[100px] h-[100px] flex items-center justify-center ">
                                {
                                    imgUrl ?
                                        <img src={imgUrl} alt="" className="w-full h-full rounded-full object-fill" />
                ...
                <div className='flex flex-col text-textDark'>
                    ...
                    {errors.name && <p className="text-bgred"> Location Name is required.</p>}
                </div>
