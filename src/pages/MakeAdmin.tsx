...
const MakeAdmin = () => {
    ...
    const handleStatusUpdate = (role: string, id: string) => {
        const updateData = {
            id, role
        }
    ...
    useEffect(() => {
        if (!isLoading && isSuccess) {
            toast.success("Make User to Admin update successful!", { toastId: 1 })
        } else if (isError) {
            toast.error("Make User to Admin update unsuccessful!", { toastId: 1 })
        }
            ...
            render: (text: string, record: any) => {
                return (
                    <div className='flex items-center justify-end'>
                        <AppModal button={
                            <button className="roundedBtn">Make Admin</button>
                        }
                            cancelButtonTitle="No, Don’t"
                            primaryButtonTitle="Yes. Make Admin"
                            primaryButtonAction={() => handleStatusUpdate("admin", record?.id)}
                        >
                            <div className='max-w-80'>
                                <p className="text-center text-[#828282] pt-4 text-lg">Are you sure make admin <span className="text-textDark font-medium">{record?.name}</span> from the user list?</p>
                            </div>
                        </AppModal>
                    </div>
                )
    ...
    return (
        <>
            <Link to={'/manage-admins'} className='text-xl 2xl:text-2xl w-fit font-medium text-[#343A40] flex items-center gap-2'>
